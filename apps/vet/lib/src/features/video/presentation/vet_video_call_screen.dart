...
class VetVideoCallScreen extends StatefulWidget {
  ...
  @override
  State<VetVideoCallScreen> createState() => _VetVideoCallScreenState();
}

class _VetVideoCallScreenState extends State<VetVideoCallScreen> {
  Room? _room;
  ...
  Future<void> _connect() async {
    try {
      ...
      final roomName = credentials['roomName']?.toString() ?? credentials['roomId']?.toString() ?? '';
  ...
  Future<void> _leaveCall() async {
    ...
    try {
      ...
      debugPrint('[VetVideo] Disconnect failed: $error');
    }
    if (!mounted) return;
    if (context.canPop()) {
      context.pop();
  ...
  Future<Map<String, dynamic>> _postGatewayJson(String path, Map<String, dynamic> body) async {
    ...
    try {
      final request = await client.postUrl(Uri.parse('${Environment.apiBaseUrl}$path'));
      ...
      children: [
        Positioned.fill(
          child: _VideoPane(
            ...
            label: remoteParticipant == null ? 'Esperando al paciente' : _participantLabel(remoteParticipant),
            large: true,
          ),
        ),
        Positioned(
...
class _VideoPane extends StatelessWidget {
  const _VideoPane({required this.participant, required this.label, this.large = false});
  ...
  Widget build(BuildContext context) {
    ...
    return DecoratedBox(
      decoration: BoxDecoration(color: Colors.white.withValues(alpha: large ? 0.04 : 0.08)),
      child: Stack(
        fit: StackFit.expand,
        children: [
          if (track != null)
            ...
            Center(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.videocam_off_rounded, color: Colors.white.withValues(alpha: 0.68), size: large ? 46 : 24),
                  const SizedBox(height: 10),
                  Text(
                    label,
        ...
        children: [
          ...
          Column(
            ...
            children: [
              const Text(
                'Videoconsulta',
                style: TextStyle(color: Colors.white, fontSize: 15, fontFamily: 'ABC Diatype', fontWeight: FontWeight.w500),
              ),
              if (roomName != null && roomName!.isNotEmpty)
                Text(
                  roomName!,
                  style: TextStyle(color: Colors.white.withValues(alpha: 0.42), fontSize: 9, fontFamily: 'ABC Diatype'),
                ),
            ],
          ),
        ],
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 10),
            _ControlButton(
              icon: ending ? Icons.hourglass_empty_rounded : Icons.call_end_rounded,
              active: true,
              destructive: true,
...
class _ControlButton extends StatelessWidget {
  const _ControlButton({required this.icon, required this.active, required this.onTap, this.destructive = false});
...
class _VideoStatusView extends StatelessWidget {
  const _VideoStatusView({required this.icon, required this.title, required this.message, this.actionLabel, this.onAction});
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              title,
              textAlign: TextAlign.center,
              style: const TextStyle(color: Colors.white, fontSize: 22, fontFamily: 'ABC Diatype', fontWeight: FontWeight.w400),
            ),
            const SizedBox(height: 8),
            Text(
              message,
              textAlign: TextAlign.center,
              style: TextStyle(color: Colors.white.withValues(alpha: 0.62), fontSize: 13, fontFamily: 'ABC Diatype'),
            ),
            if (actionLabel != null && onAction != null) ...[
              const SizedBox(height: 18),
              TextButton(onPressed: onAction, child: Text(actionLabel!, style: const TextStyle(color: Colors.white))),
            ],
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: const Color(0xFFFF4438).withValues(alpha: 0.34)),
      ),
      child: Padding(
        ...
        child: Text(
          message,
          textAlign: TextAlign.center,
          style: const TextStyle(color: Colors.white, fontSize: 11, fontFamily: 'ABC Diatype'),
        ),
      ),
    );
  }
...
bool _hasActiveVideo(Participant? participant) => _activeVideoTrack(participant) != null;
...
class _VetVideoException implements Exception {
  const _VetVideoException(this.message);
