...
declare
  ...
  v_per_day_limit integer := case when p_channel = 'sms' then 8 else 10 end;
  v_ip_window_limit integer := 20;
  v_ip_day_limit integer := 100;
begin
  ...
  if v_per_day_count >= v_per_day_limit then
    return query select
      false,
      'daily_cap_reached',
      'alcanzaste el máximo diario de códigos. intenta mañana.',
      86400;
    return;
  end if;
