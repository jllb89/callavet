...
CREATE TABLE IF NOT EXISTS users (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(full_name,'') || ' ' || coalesce(email,'') || ' ' || coalesce(phone,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS pets (
  ...
  created_at timestamp DEFAULT now(),
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(name,'') || ' ' || coalesce(species,'') || ' ' ||
              coalesce(breed,'') || ' ' || coalesce(medical_notes,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS vet_specialties (
  ...
  description text,
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(name,'') || ' ' || coalesce(description,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS vets (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(
      coalesce(bio,'') || ' ' || coalesce(license_number,'') || ' ' ||
      coalesce(country,'') || ' ' || coalesce(array_to_string(languages,' '),'')
    )
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS products (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(
      ...
      coalesce(array_to_string(meta_words,' '),'') || ' ' ||
...
CREATE TABLE IF NOT EXISTS service_providers (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(
      coalesce(name,'') || ' ' || coalesce(description,'') || ' ' ||
      coalesce(type,'') || ' ' || coalesce(website,'') || ' ' || coalesce(location,'')
    )
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS vet_care_centers (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(name,'') || ' ' || coalesce(description,'') || ' ' ||
              coalesce(address,'') || ' ' || coalesce(website,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS consultation_notes (
  ...
  created_at timestamp DEFAULT now(),
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(summary_text,'') || ' ' || coalesce(plan_summary,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS care_plans (
  ...
  created_at timestamp DEFAULT now(),
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(short_term,'') || ' ' || coalesce(mid_term,'') || ' ' || coalesce(long_term,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS care_plan_items (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(type,'') || ' ' || coalesce(description,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS global_pet_health_data (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(
      coalesce(species,'') || ' ' || coalesce(breed,'') || ' ' || coalesce(sex,'') || ' ' ||
      coalesce(geo_region,'') || ' ' || coalesce(diagnosis_label,'') || ' ' ||
      coalesce(array_to_string(symptoms,' '),'') || ' ' || coalesce(notes,'')
    )
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS kb_items (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(
      ...
      coalesce(array_to_string(species,' '),'') || ' ' ||
      coalesce(array_to_string(tags,' '),'')
    )
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS subscription_plans (
  ...
  updated_at timestamp DEFAULT now(),
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(name,'') || ' ' || coalesce(description,'') || ' ' || coalesce(code,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS user_subscriptions (
  ...
  updated_at timestamp DEFAULT now(),
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(status,'') || ' ' || coalesce(provider,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  created_at timestamp DEFAULT now(),
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(coalesce(provider,'') || ' ' || coalesce(status,''))
  ) STORED
);
...
CREATE TABLE IF NOT EXISTS image_cases (
  ...
  search_tsv tsvector GENERATED ALWAYS AS (
    es_en_tsv(
      ...
      coalesce(findings,'') || ' ' || coalesce(diagnosis_label,'')
    )
  ) STORED
);
